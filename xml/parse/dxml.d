...
class DXmlParser(T)  {
	...
	private
	{
		...
		final void unpop(char makeFront)
		{
			assert(makeFront < 0x80);

			auto blen = backStack.length;
			if (blen > 0)
			{
			...
			if (slicing_)
			{
				// Current front is unknown size of unicode character, from mpos to fpos, to be pushed back by a markup character
				// assume can go back where it came from
				if (mpos < sliceData_.length)
				{
					auto pos = mpos;
					dchar test = decode(sliceData_,pos);
					if (front == test)
					{
						fpos = mpos;
						mpos--;
		...
		final bool matchInput(string match)
		{
			...
			{
				foreach_reverse(d;bufMatch.peek)
					unpop(d);
				return false;
			}
		}
		...
		Buffer!(immutable(T)[])		entityStack_;
		...
		// for none allocation purposes
		final bool getXmlName(ref Buffer!T tag)
		{
			if (empty)
				return false;
			auto test = front;
		...
		final bool getXmlName(ref immutable(T)[] tag)
		{
			if (empty)
				return false;
			auto test = front;
		...
		final void unquoteValue(ref immutable(T)[] val)
		{
		...
		final void getPIData(ref Buffer!T app)
		{
			dchar  test  = 0;
		...
		void doXmlDeclaration()
		{
			...
			if (hasXmlVersion)
			{
				...
				attributes_ ~= Attribute("version",xmlStrValue);
			}
			markupDepth--;
			...
			if (hasEncoding)
			{
				...
				attributes_ ~= Attribute("encoding", encodingValue);
			}
			if (hasStandalone)
			{
			...
			while (true)
			{
				...
				switch(front)
				{
					...
					default:
						...
						attributes_ ~= Attribute(attrName_,attrValue_);
	...
	@property {
		...
		final XmlEvent!T eventReturn()
		{
			return results_;
		}
		final void docInterface(IXmlDocHandler!T pdata)
		{
			events_ = pdata;
		}
		final void errorInterface(IXmlErrorHandler!T pdata)
		{
			errors_ = pdata;
		}
	}
    ...
    void popContext()
    {
        ...
        if (slen > 0)
        {
			...
			backStack.takeOver(ctx_.backStack);	 // oops wrong way buffer
    ...
    protected bool doReplaceCharRef(ref Buffer!T app)
    {
        dchar rchar = 0;
		auto beginLength = app.length;
        ...
        while (!empty)
        {
            ...
            if (test == '&')
            {
                ...
                else
                {
                    ...
                    app.put('&');
                    if (!getXmlName(bufTag_) || !matchInput(';'))
						throw errors_.makeException(XmlErrorCode.BAD_ENTITY_REFERENCE);
                    app.put(bufTag_.peek);
                    app.put(';');
                }
            }
            else
        ...
        return (app.length - beginLength > 0);
    }
	...
	void expandParameterEntities(bool isInternal, ref Buffer!T app, ref StringSet stk)
    {
        XmlString evalue;
				...
				case '%':
					popFront();
					{
						if (!empty && isNameStartFn(front))
						{
							if (!getXmlName(bufTag_) || !matchInput(';'))
								throw errors_.makeException(XmlErrorCode.BAD_ENTITY_REFERENCE);
							EntityData ed = getParameterEntity(bufTag_.peek,stk,true);
							if (ed is null)
								throw errors_.makeException(format("Parameter entity %s not found", bufTag_.peek));
							app ~= ed.value_;
						}
					}
					break;
    ...
    void attributeTextReplace(ref Buffer!T app, uint callct = 0)
    {
	NEXT_CHAR:
        while(!empty)
        {
			...
			switch(test)
			{
				...
				case '&':
					...
					auto pc = bufTag_.peek in charEntity;
					if (pc !is null)
					{
						app.put(*pc);
					}
					else
					{
						...
						if (ed !is null)
						{
							if (!ed.isInternal_ && this.isStandalone_)
								throw errors_.makeException(format("External entity %s referenced from standalone",bufTag_.peek));
		...
		while(!empty)
        {
			...
			case '&':
                ...
                if (front == '#')
                {
                    uint radix;
                    dchar cref = expectedCharRef(radix);
                    bufNormAttr_.put(cref);
                    continue;
                }
                if (!getXmlName(bufTag_) || !matchInput(';'))
                    noSingleAmp();
                auto pc = bufTag_.toArray in charEntity;
                if (pc is null)
                {
                    pushEntity(bufTag_.peek, true);
                }
                else
                {
                    bufNormAttr_.put(*pc);
				...
				break;
			default:
                bufNormAttr_.put(front);
                popFront();
				break;
            }
        }
		src = bufNormAttr_.idup;
    }
	...
	version(ParseDocType)
	{
		...
		private bool getSystemLiteral(ref XmlString opt)
		{
			...
			if (opt.indexOf('#') >= 0)
				throw errors_.makeException("SYSTEM URI with fragment '#'");
			return true;
		}
		...
		final bool getUpperKeyWord(ref Buffer!T kw)
		{
			kw.clear();
			while(true)
			{
				...
				if (std.ascii.isUpper(test))
				{
					...
					if (std.ascii.isAlphaNum(test) || (kw.length == 0))
						throw errors_.makeException(format("Upper case keyword expected: %s + %s", kw.peek, test));
					return kw.length > 0;
				}
			}
		}
			...
			while(!empty)
			{
				...
				if (isOpenBang())
				{
					...
					if (matchInput("--"))
					{
						...
						// A key word expected
						if (!getUpperKeyWord(bufTag_))
							throw errors_.makeException(format("Uppercase key word expected, not %s", bufTag_.peek));
						switch(bufTag_.peek)
						{
						case "ENTITY":
							parseEntity();
						...
						case "NOTATION":
							...
							break;
						default:
							throw errors_.makeException(format("DTD unhandled keyword %s ",bufTag_.peek));
						}
					}
					errors_.checkErrorStatus();
					continue;
		...
		protected final void expectEntityName(ref Buffer!T ename)
		{
			if (!getXmlName(ename))
				throw errors_.makeException(XmlErrorCode.BAD_ENTITY_REFERENCE);
			if (empty || front != ';')
		...
		{
			expectEntityName(bufTag_);
			if (!allowed)
			{
				// get the entity name and say its not allowed here
				throw errors_.makeException(format("parameter entity not allowed in internal subset: %s",bufTag_.peek));
			}
			StringSet eset;
			EntityData ed = getParameterEntity(bufTag_.peek, eset, isValue);

			if (ed is null)
			{
				throw errors_.makeException(format("Unabled to fetch entity %s",bufTag_.peek),XmlErrorLevel.ERROR);
			}
			return ed;
		}
	...
	void verifyParameterEntity(XmlString s, bool isPE)
    {
        ...
        while (!empty)
        {
            if (front == '%')
            {
                ...
                {
                    EntityData eref = dtd_.paramEntityMap[bufTag_.peek];
                    if (eref is null)
                    {
                        bombWF(s);
                    }
        ...
        while(endless)
        {
            ...
            switch(front)
            {
				case '#':
					...
					{
						return fatalError(format("unknown #",bufTag_.peek));
					}
					defct++;
					expectSeparator = true;
					expectListItem = false;
		...
		*/
        ...
        switch(reftype)
        {
			...
			case RefTagType.NOTATION_REF:
				if (inGeneralEntity())
					throw errors_.makeException(format("Referenced unparsed data in entity %s", getEntityName()));
    ...
    private bool deriveEntityContent(EntityData entity, StringSet stk, out int reftype)
    {
        ...
        if (isStandalone())
        {
            ...
            {
                errors_.pushError("Standalone yes and referenced external entity",XmlErrorLevel.ERROR);
                return false;
            }
	...
	protected bool textReplaceCharRef(ref Buffer!T app)
    {
        uint radix;
        dchar rchar = 0;
				...
				case '&':
					...
					else
					{
						...
						else
						{
							int ref2;
							XmlString evalue;
							if (entityNameSet.contains(bufTag_.peek))
								throw errors_.makeException(XmlErrorCode.CIRCULAR_ENTITY_REFERENCE);
							if (!lookupReference(bufTag_.idup, entityNameSet,evalue, ref2))
							{
								return false;
							}
							else
    ...
    void parseDtdInclude()
    {
		...
		auto peek = bufTag_.peek;
        if (peek == "INCLUDE")
        {
            if (dtd_.isInternal_ && isInternalContext)
                throw errors_.makeException(format(kNotAllowed, peek));
            munchSpace();
    ...
    final void checkBalanced()
    {
		...
		auto balanceMismatchMsg()
		{
			...
			if (markupDepth != 0)
				msg.put("Mismatch of < > ");
			if (elementDepth != 0)
				msg.put("Mismatch of element depth");
			if (parenDepth != 0)
				msg.put("Mismatch of ( )");
			return msg.peek;
		}
    ...
    int parseAttList()
    {
        ...
        auto ename = bufTag_.idup;
        AttributeList def = dtd_.attributeListMap.get(ename,null);
        ...
        if (def is null)
        {
            ...
            dtd_.attributeListMap[ename] = def;
        }
        ...
        ElementDef edef = dtd_.elementDefMap.get(ename,null);
        ...
        while (true)
        {
            ...
            AttributeDef adef = new AttributeDef(bufTag_.idup);
            ...
            else
            {
                ...
                AttributeType*  patte = bufTag_.peek in AttributeDef.stdAttTypeList;

                if (patte is null)
                {
                    throw errors_.makeException(text("Unknown attribute type in ATTLIST ",bufTag_.peek));
                }
                adef.dataform = *patte;
        ...
        while(!empty)
        {
            if (front == '&')
            {
                ...
                else
                {
                    ...
                    auto refName = bufTag_.peek;
                    auto pc = refName in charEntity;
                    if (pc is null)
                    {
                        EntityData edef = dtd_.getEntity(bufTag_.peek);
                        if (edef is null)
                        {
                        ...
                        {
                            errors_.makeException( text("Cannot use notation entity as value: ", refName));
                        }

                        if (edef.src_.systemId_.length > 0)
                        {
                            errors_.makeException( text("Cannot use external entity as value: ", refName));
                        }
                    }
                }
            }
    ...
    private bool getAttributeDefault(ref AttributeDefault dft)
    {
        ...
        if (dfkey == "REQUIRED")
            ...
            throw errors_.makeException( text("Unknown attribute specification ",dfkey));
        return true;
    }

    private final bool getXmlNmToken(ref Buffer!T cbuf)
    {
        if (empty)
            return false;
