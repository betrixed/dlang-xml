...
class DXmlDomBuild(T) : xmlt!T.IXmlErrorHandler, xmlt!T.IXmlDocHandler
{
    ...
    final void instruction(const XmlEvent  s)
    {
               ...
               if (s.data == "POP_ENTITY")
               {
                    DocumentFragment df = cast(DocumentFragment) level_.n_;
                    if (df is null)
                    {
                        // a stuff up exists
                        throw makeException("Entity POP stack error");
                    }
                    popWorkingStack();
               }
               else {
    ...
    void entityRef(const XmlEvent s){
          EntityReference n = doc_.createEntityReference(s.data);
          level_.n_.appendChild(n);
          checkEntity(n);
    }

    void checkEntity(EntityReference eref)
    {
        if (dtdNode_)
        {
            NamedNodeMap nmap = dtdNode_.getEntities();
            auto ename = eref.getNodeName();
            if (nmap.getNamedItem(ename))
            {
                return; // all good, we've done this one already
            }
        // set up another parser object, or hijack this one?
        // this time to parse a document fragment
        // first make sure we have something to parse. Fortunately, the parser can do some of this work,
        // as if the entity data was inline, just after its reference.

        // Problems with hijacking existing parser - the class Entity is an idNode, with its own xmlversion,
        // encoding. End of context, with popContext is a tricky event.

        // If a new parser is created then need a different DOM Builder to
        // parse the data, almost the same as this one, with all sorts of validation issues.

        // The issue is this object stack_, and what's on it to take the standard nodes, and when it gets popped off
        // The Entity document fragment gets pushed on the stack_, so it can take all its child nodes.

            EntityData edata =  parser_.getEntityData(ename,false);
            if (edata is null)
            {
                // TODO: throw a hissy?
                checkErrorStatus();
            ...
            auto evalue = edata.value_;
            if (evalue.length > 0)
            {
                // parser will now continue on merrily, until parser popFront discovers that
                // a popContext is necessary.   popContext is more than likely to be in middle of content.
                // popContext of the entity data, has to save any parser_ inCharData, bufContent, as a Text node to
                // be added to the stack_ DocumentFragment here, before the stack_ DocumentFragment gets popped!
                // Weird idea solution, append a processing instruction for self, to the entity processing text, to indicate this break,
                // this will pre-empt the popContext, cause any content to be saved, and provide a check on the stack_ level
                evalue = evalue ~ to!(immutable(T)[])("<?POP_ENTITY ?>");
                Entity e = new Entity(ename, edata.src_.systemId_);
                e.encoding_ = edata.encoding_;
                e.version_ = edata.version_;
                ...
                stack_ ~= level_;
                level_ = ElementLevel.init;
                level_.n_ = df;


                parser_.pushContext(evalue,false,edata);
