...
alias Buffer!Variant ValueStack;
...
struct EvalOp
{
    ...
    {
        mop = op;
        mop2 = EvalOpType.ValNone;
    }

    this(EvalOpType op, ref Variant cdata)
    {
        mop = op;
        mop2 = EvalOpType.ValNone;
        data = cdata;
    }
    this(EvalOpType op, EvalOpType op2, ref Variant cdata)
    {
        mop = op;
        mop2 = op2;
        data = cdata;
...
private struct EvalPathStep
{
    ...
    NodeList process(NodeList source, PathStep pe)
    {
        source_ = source;
        pathStep_ = pe;
        foreach(Node n; source_)
        {
            axisShortList_.length = 0;
            contextNode_ = cast(Element) n;
            if (contextNode_ !is null)
            {
                axisApply();
            }
            if (axisShortList_.length > 0)
                result_.put(axisShortList_.data);
...
struct XPathParser
{
        ...
        if (qual >= 0)
        {
            if (op == TokenKind.NodeType)
            {
                ...
                else
                {
                    // quoted argument
                    scratch.reset();
                    if (src.unquote(scratch))
                    {
                        name = scratch.data.idup;
                    }
                    else
                    {
    ...
    private void getAxis(ParseSource src, bool checkFunction)
    {
        ...
        bool gotQName()
        {
            return src.getQName(scratch,prefix);
        }
TRY_AGAIN:
        switch(src.front)
        {
        ...
        default:
        {
            ...
            if (prefix < 0)
            {
                ...
                else if (checkFunction && src.match('('))
                {
    ...
    private void getPredicate(ParseSource src)
    {
        ...
        while(!src.empty)
        {
            ...
            if (inExpression)
            {
                ...
                case '.':
                    ...
                    if (!src.getQName(scratch,prefix))
                        throw new XPathSyntaxError("Expect node test");
                    nodeTest(src,scratch.data,prefix);
                    break;
                case '@':
                    src.popFront;
        ...
        StepEval toPostfix(ref ExpToken[] src)
        {
            ...
            while (src.length > 0)
            {
                ...
                if (et.kind == TokenKind.Operator)
                {
                    ...
                    if (opStack.length==0)
                        opStack.put(EvalOp(cast(EvalOpType)op));
                    else
                    {
                        // compare precedence
                ...
                else   // some operand
                {
                    ...
                    case TokenKind.AxisName:
                        v1 = Variant(toPath(src));
                        pfset.put(EvalOp(EvalOpType.ValPath, v1));
                        doPop = false;
                        break;
                    ...
                    case TokenKind.Number:
                        double val = to!double(et.vdata.get!string());
                        v1 = val;
                        pfset.put(EvalOp(EvalOpType.ValNumber, v1));
                        break;
                    ...
                    case TokenKind.Function:
                        ...
                        if (xfc.returnType == StepEval.SE_INDEX)
                            isIndex = true;
                        pfset.put(EvalOp(EvalOpType.FunctionCall,
                                    cast(EvalOpType) et.qualifier, et.vdata));
                        break;
                    default:
        ...
        {
            auto sf = new InputArray!char(s);

            ParseSource ps = new ParseSource(sf);
