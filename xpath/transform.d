...
class XSLTransform {
	...
	*/
	...
	{
		XmlPrintOptions	outopt;
		outopt.indentStep = 0; // no auto indent;
		auto tp = XmlPrinter(outopt);
		...
		while (ch !is null)
		{
			...
			case NodeType.Element_node:
				...
				if (prefix == "xsl")
				{
					...
					if (fname == "for-each")
					{
						...
						if (results.getLength() > 0)
						{
							...
							if (sortOrder !is null)
							{
								...
								auto keyset = nb.keys().sort!("a<b");
								Node[] valueSet = new Node[keyset.length];
								uintptr_t kix = 0;
								foreach(k;keyset)
								{
									valueSet[kix] = nb[k];
					...
					XmlAttributeMap smap = XMLDOM!char.toAttributeMap(e);
					tp.putStartTag(tag,smap,!hasChildren);
					if (hasChildren)
					{
						processTemplate(e,contextNode);
						tp.putEndTag(tag);
